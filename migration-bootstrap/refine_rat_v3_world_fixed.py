...
def reshape_body_v3():
    ...
    for v in obj.data.vertices:
        ...
        if z > 2.55:
            ...
            # Increase lower-face projection enough to clear the established
            # sculpt gate while keeping the deformation local to the central muzzle.
            co.y -= 0.21 * front * lower_face * (0.45 + 0.55 * center)
...
def combined_group_weight(obj, vertex, group_names):
    total = 0.0
...
def compute_metrics():
    ...
    verts = [(v, to_world @ v.co) for v in body.data.vertices]
    head = [p for _, p in verts if p.z > 2.55]

    # The body is a continuous mesh that includes the arms. Z-only width bands
    # incorrectly counted low arm/hand vertices as waist. Measure torso taper
    # only from vertices whose deformation is primarily controlled by trunk bones.
    trunk_groups = {'hips', 'spine', 'chest', 'neck'}
    torso_verts = [
        (v, p)
        for v, p in verts
        if combined_group_weight(body, v, trunk_groups) >= 0.50
    ]
    shoulders = [p for _, p in torso_verts if 2.16 <= p.z <= 2.42]
    waist = [p for _, p in torso_verts if 1.62 <= p.z <= 1.82]
    ...
    metrics = {
        ...
        'torso_metric_min_trunk_weight': 0.50,
    }
